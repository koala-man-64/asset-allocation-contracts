...
foreach ($row in $contractRows) {
    ...
    if ($name -eq "DISPATCH_APP_PRIVATE_KEY" -and $null -ne $dispatchPrivateKeyResolution) {
        $results.Add([pscustomobject]@{ Name = $name; Value = $dispatchPrivateKeyResolution.Value; Source = $dispatchPrivateKeyResolution.Source; IsSecret = $true; PromptRequired = $false })
        continue
    }
